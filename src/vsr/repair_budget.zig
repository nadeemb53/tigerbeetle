...
pub const RepairBudgetJournal = struct {
    ...
    pub fn decrement(
        ...
        budget.reap_expired_requests(now);
    ...
    fn reap_expired_requests(budget: *RepairBudgetJournal, now: stdx.Instant) void {
        budget.assert_invariants();
        defer budget.assert_invariants();
...
pub const RepairBudgetGrid = struct {
    ...
    pub fn next_destination(budget: *RepairBudgetGrid, prng: *stdx.PRNG, now: stdx.Instant) ?u8 {
        budget.assert_invariants();
        defer budget.assert_invariants();
    ...
    fn reap_expired_requests(budget: *RepairBudgetGrid, now: stdx.Instant) void {
        budget.assert_invariants();
        defer budget.assert_invariants();
